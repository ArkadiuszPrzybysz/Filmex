...
<head>
    ...
    <link rel="stylesheet" href="movie-genre.css">
...
<body>
        ...
        <nav class="navbar">
                ...
                <ul class="nav-list">
                    ...
                    while($row = $wynik->fetch_assoc()){
                        ...
                        if($row["genre_id"] == $id){
                            echo "<li class='nav-item link-main'>";
                        }else{
    ...
    <?php
                    ...
                    $wynik = $connect->query("SELECT genre_name FROM genre WHERE genre_id = $id");

                    while($row = $wynik->fetch_assoc()){
                        echo "<h1>".$row["genre_name"]."</h1>";
                    }
                    $connect -> close(); 
                    ?>
        <div class="similar-all">
            ...
            <?php
                    ...
                    $wynik = $connect->query("SELECT DISTINCT movies.poster, movies.title, movies.movie_id FROM movie_genres INNER JOIN movies ON movie_genres.movie_id = movies.movie_id WHERE genre_id = $id");
